...
;;   the remainder is deployed to the Bitflow sBTC-STX XYK pool.
;;   Two-path withdraw:
...
(define-constant BITFLOW_SBTC_STX_POOL 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1)
(define-constant STX_WRAPPER_TOKEN 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.token-stx-v-1-2)
...
(define-private (transfer-in (token principal) (amount uint))
    (begin
        (if (is-eq token STX_TOKEN)
            (unwrap! (stx-transfer? amount tx-sender .satsflow-streams-v7) ERR_TRANSFER_FAILED)
            (unwrap!
                (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
                    transfer amount tx-sender .satsflow-streams-v7 none)
                ERR_TRANSFER_FAILED
            )
        )
        (ok true)
...
;; bitflow-deploy -- deploy sBTC to Bitflow sBTC-STX XYK pool.
;; Sequence:
;;   1. Swap deploy-amount/2 sBTC -> STX-wrapper via xyk-core swap-x-for-y
;;   2. Add liquidity with deploy-amount/2 sBTC (XYK auto-computes STX-wrapper needed from current ratio)
;;   3. Residual STX-wrapper remains in contract; swapped back on bitflow-unwind
...
(define-private (bitflow-deploy (deploy-amount uint))
    (let
        ...
        (begin
            ;; Step 1: swap half sBTC -> STX-wrapper to seed the y-token side
...
;;   2. swap-y-for-x (STX-wrapper -> sBTC)
